...
const makePr = (id: number): PurchaseRequest => ({
  id, prNumber: `PR-2026-000${id}`, title: `Item ${id}`, status: 'submitted',
  totalEstimatedAmount: 1000, quarter: null, requiredDate: '2026-02-01', requesterId: id,
  requester: { id, fullName: `User ${id}`, email: `u${id}@x.com` }, departmentId: 1,
...
describe('ApprovalQueue', () => {
  ...
  it('renders submitted PR rows with links to detail', () => {
    vi.mocked(useApprovalQueue).mockReturnValue({ data: [makePr(1)], isLoading: false } as ReturnType<typeof useApprovalQueue>)
    renderQueue()
    expect(screen.getByText('PR-2026-0001')).toBeInTheDocument()
    expect(screen.getByRole('link', { name: /ดู \/ อนุมัติ/ })).toHaveAttribute('href', '/purchase-requests/1')
  })
  ...
  it('shows "ดูทั้งหมด" link when 5 items returned (may be more)', () => {
    vi.mocked(useApprovalQueue).mockReturnValue({ data: [1,2,3,4,5].map(makePr), isLoading: false } as ReturnType<typeof useApprovalQueue>)
    renderQueue()
    expect(screen.getByRole('link', { name: /ดูทั้งหมด/ })).toHaveAttribute('href', '/purchase-requests?status=submitted')
  })
})
