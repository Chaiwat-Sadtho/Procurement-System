...
function mkPR(over: Partial<PurchaseRequest>): PurchaseRequest {
  return {
    id: 1, prNumber: 'PR-2026-0001', title: 'Item', status: 'draft',
...
function mockAttention(drafts: PurchaseRequest[], rejected: PurchaseRequest[]) {
  vi.mocked(useAttentionPRs).mockReturnValue({
    data: { drafts, rejected }, isLoading: false,
  } as ReturnType<typeof useAttentionPRs>)
}

function renderList() {
  return render(<MemoryRouter><AttentionList /></MemoryRouter>)
}
...
describe('AttentionList', () => {
  ...
  it('lists rejected PRs linking to detail, with ดูทั้งหมด to the filtered list', () => {
    mockAttention([], [mkPR({ id: 9, prNumber: 'PR-2026-0009', title: 'Chair', status: 'rejected' })])
    renderList()
    expect(screen.getByText(/Rejected รอแก้/)).toBeInTheDocument()
    expect(screen.getByRole('link', { name: /PR-2026-0009/ })).toHaveAttribute('href', '/purchase-requests/9')
    expect(screen.getByRole('link', { name: /ดูทั้งหมด/ })).toHaveAttribute('href', '/purchase-requests?status=rejected')
  })
