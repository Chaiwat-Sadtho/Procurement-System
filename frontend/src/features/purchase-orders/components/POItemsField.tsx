...
import { emptyItem, safeNum, type POFormValues } from '../lib/poFormSchema'

interface POItemsFieldProps {
  form: UseFormReturn<POFormValues>
}

export function POItemsField({ form }: POItemsFieldProps) {
  const { control } = form
  ...
  return (
    <div className="space-y-4">
      ...
      {fields.map((field, index) => {
        const rowTotal = safeNum(watched?.[index]?.quantity) * safeNum(watched?.[index]?.unitPrice)
        return (
