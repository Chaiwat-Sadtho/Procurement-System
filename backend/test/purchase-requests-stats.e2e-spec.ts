...
describe('Purchase Requests Stats (e2e)', () => {
  ...
  async function createPr(token: string): Promise<number> {
    const res = await http().post('/api/v1/purchase-requests')
  ...
  async function getStats(token: string) {
    const res = await http().get('/api/v1/purchase-requests/stats')
      .set('Authorization', `Bearer ${token}`).expect(200);
    return res.body as { total: number; draft: number; submitted: number; approved: number; rejected: number };
  }
  ...
  beforeAll(async () => {
    ...
    await http().post(`/api/v1/purchase-requests/${toSubmit}/submit`).set('Authorization', `Bearer ${empAToken}`).expect(201);
    const toApprove = await createPr(empAToken);
    await http().post(`/api/v1/purchase-requests/${toApprove}/submit`).set('Authorization', `Bearer ${empAToken}`).expect(201);
    await http().post(`/api/v1/purchase-requests/${toApprove}/approve`).set('Authorization', `Bearer ${mgrAToken}`).expect(201);
    const toReject = await createPr(empAToken);
    await http().post(`/api/v1/purchase-requests/${toReject}/submit`).set('Authorization', `Bearer ${empAToken}`).expect(201);
    await http().post(`/api/v1/purchase-requests/${toReject}/reject`).set('Authorization', `Bearer ${mgrAToken}`).send({ reason: 'no' }).expect(201);

    // emp B (dept B): submitted=1 (เพื่อพิสูจน์ manager A ไม่เห็นแผนกอื่น)
    const bPr = await createPr(empBToken);
    await http().post(`/api/v1/purchase-requests/${bPr}/submit`).set('Authorization', `Bearer ${empBToken}`).expect(201);
  });
  ...
  it('employee sees only own PR counts', async () => {
    ...
    expect(s).toEqual({ total: 4, draft: 1, submitted: 1, approved: 1, rejected: 1 });
  });
  ...
  it('returns all zeros when user has no PRs', async () => {
    ...
    expect(s).toEqual({ total: 0, draft: 0, submitted: 0, approved: 0, rejected: 0 });
  });
});
